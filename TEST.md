...
##### TaskForm 设为默认配置（2026-09-06）

| 测试 | 验证内容 |
|------|---------|
| `TaskForm.test.tsx::restores stored defaults after creating a task` | 创建任务后表单重置到 localStorage 保存的默认值，而非服务器默认值 |
| `TaskForm.test.tsx::clearDefault hides the clear button immediately` | 点击「清除默认」后按钮立即消失，localStorage 被清除 |
| `TaskForm.test.tsx::clearDefault resets form to server defaults` | 清除默认后表单重置为服务器默认值（provider/effort/timeout 等） |
| `TaskForm.test.tsx::Config button highlights when systemPromptMode is non-default` | systemPromptMode 非默认时 Config 按钮高亮（amber）|
