...
│   │   ├── cloudrouter_accounts.py # CloudRouter API 账号/模型/额度管理 API
...
- **Skills 系统**: Task.enabled_skills（JSON dict，如 `{"monitor": true}`）控制注入哪些 MCP 工具。启用只表示能力可发现，不等于用户已经调用；只有消息开头真实解析到 `$command` 时才注入显式调用指令。新建 Task 的 description 以已注册 `$command` 开头时，dispatcher 必须像 chat 续轮一样去掉命令前缀，并在 execution ownership commit 中临时写入 required skills，使首个 MCP 调用能通过 API gate；turn 结束后仅在值仍等于临时快照时恢复原 enabled_skills，避免覆盖并发用户修改。创建 task 时勾选 Skills，dispatcher 根据 enabled_skills 动态生成 provider 对应的 MCP config。聊天页的任务级 Distill 必须按 `Task.provider` 分流；Codex 使用绑定/健康账号运行独立 `codex exec --ephemeral`，不得复用或改写原 task 的 thread/账号绑定。该临时进程必须通过 `InstanceManager.codex_home_exec_guard` 纳入同一 CODEX_HOME maintenance 准入，取消/超时须 kill + reap 后才释放 home；对话属于不可信输入，Codex distill 必须使用只读 sandbox 并忽略用户 config/rules，严禁 `--dangerously-bypass-approvals-and-sandbox`
...
- **MCP 架构**: `mcp_config.py` 先为主任务、Monitor、Sub-Agent 构造 provider-neutral `McpServerSpec`（command/args/cwd/env/required/enabled_tools/timeouts），再由 provider renderer 输出配置：Claude renderer 生成兼容的 `mcpServers` JSON；Codex renderer 生成 app-server 线程级 `config.mcp_servers` 对象，或从同一对象生成 `codex exec -c` TOML argv（纯内存、不写全局 `config.toml`）。Codex 主任务 MCP 默认启用；`CODEX_MAIN_MCP_ENABLED=false` 是紧急关闭开关。`InstanceManager` 每次 start/resume/重试/换号都重建 task-scoped `ccm_skills` required spec，并把同一份 spec 交给 app-server 或 exec。invalid config、turn/start 及之后的失败、未知 required-MCP 异常一律 fail closed；只有 transport 启动或 `thread/start|resume` 在 turn 提交前失败才标为 `CodexRequiredMcpPreTurnError`，且仅完整主任务 MCP 可回退到 MCP-equivalent exec。Sub-Agent 因需要 live thread control 始终只走 app-server，即使同一任务也开启完整主 MCP。Codex 的普通 Skill prompt/UI 仍未开放。三个 FastMCP server 分别是 `ccm_skills_server.py`、`ccm_monitor_agent_server.py`、`ccm_sub_agent_server.py`，通过 stdio 与 CLI 通信、通过 HTTP 调用 CCM API。Claude 配置文件动态生成到临时目录并在对应进程结束后清理
