...
vi.mock('../../api/client', () => ({
  api: {
    ...
    config: vi.fn().mockResolvedValue(DEFAULT_MOCK_CONFIG),
...
describe('Codex provider UI gating', () => {
  ...
  it('shows Monitor with ordinary/User Skills for local Codex without a status label', async () => {
    const cliSelect = await renderAndOpenConfig();
...
describe('TaskForm persisted defaults', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    // Restore the default config mock — earlier describe blocks may have overridden it
    vi.mocked(api.config).mockResolvedValue(DEFAULT_MOCK_CONFIG);
  });
  ...
  it('restores stored defaults after creating a task', async () => {
    localStorage.setItem('cc_default_task_config', JSON.stringify({
      provider: 'codex',
      model: 'gpt-5.6-sol',
      effort: 'high',
    ...
    // Wait for config to load and verify stored defaults are applied
    await waitFor(() => {
      expect(screen.getByDisplayValue('gpt-5.6-sol')).toBeInTheDocument();
      expect(screen.getByDisplayValue('high')).toBeInTheDocument();
      expect(screen.getByDisplayValue('4 hours')).toBeInTheDocument();
    });

    // Override effort for this one task
    await userEvent.selectOptions(screen.getByDisplayValue('high'), 'low');
    expect(screen.getByDisplayValue('low')).toBeInTheDocument();

    // Close config, type description, submit
    await userEvent.click(screen.getByText('Config'));
    await userEvent.type(
      screen.getByPlaceholderText('Prompt / Description (this will be sent to Codex)'),
      'test task',
    ...
    // Wait for form to fully reset after task creation
    await waitFor(() => {
      expect(api.createTask).toHaveBeenCalled();
      expect(screen.getByPlaceholderText(/Prompt/)).toHaveValue('');
    });

    // Reopen config — stored defaults should be restored (effort back to 'high')
    await openConfigPanel();
    await waitFor(() => {
      expect(screen.getByDisplayValue('Codex')).toBeInTheDocument();
      expect(screen.getByDisplayValue('gpt-5.6-sol')).toBeInTheDocument();
  ...
  it('clearDefault hides the clear button immediately', async () => {
    localStorage.setItem('cc_default_task_config', JSON.stringify({
      provider: 'codex',
      effort: 'high',
    }));
    ...
    render(<TaskForm onCreated={vi.fn()} />);
    await openConfigPanel();
    // Wait for config to load so effort options are available in the select
    await waitFor(() => expect(screen.getByDisplayValue('high')).toBeInTheDocument());

    // "清除默认" should be visible
    expect(screen.getByRole('button', { name: '清除默认' })).toBeInTheDocument();

    // Click it
    await userEvent.click(screen.getByRole('button', { name: '清除默认' }));

    // The button should disappear immediately
    expect(screen.queryByRole('button', { name: '清除默认' })).not.toBeInTheDocument();
    expect(localStorage.getItem('cc_default_task_config')).toBeNull();
  });

  it('clearDefault resets form to server defaults', async () => {
    localStorage.setItem('cc_default_task_config', JSON.stringify({
      provider: 'codex',
      effort: 'high',
      timeoutHours: '4',
    ...
    // Wait for config to load and stored defaults applied
    await waitFor(() => {
      expect(screen.getByDisplayValue('Codex')).toBeInTheDocument();
      expect(screen.getByDisplayValue('high')).toBeInTheDocument();
      expect(screen.getByDisplayValue('4 hours')).toBeInTheDocument();
    ...
    // Clear defaults — form should reset to server defaults
    await userEvent.click(screen.getByRole('button', { name: '清除默认' }));

    await waitFor(() => {
      // Server default_provider is 'claude', so provider resets
      expect(screen.getByDisplayValue('Claude')).toBeInTheDocument();
      // effort and timeout should be back to defaults
      expect(screen.getByDisplayValue('medium (default)')).toBeInTheDocument();
      expect(screen.queryByDisplayValue('4 hours')).not.toBeInTheDocument();
    });
  });

  it('Config button highlights when systemPromptMode is non-default', async () => {
    localStorage.setItem('cc_default_task_config', JSON.stringify({
    ...
    // Wait for stored defaults to be applied and reflected in the Config button
    await waitFor(() => {
      const configBtn = screen.getByText('Config').closest('button')!;
      expect(configBtn.className).toContain('amber');
    });
  });
});
