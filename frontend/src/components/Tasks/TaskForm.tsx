...
export function TaskForm({ onCreated }: TaskFormProps) {
  ...
  const clearDefault = () => {
    ...
    applyStoredDefaults(null, defaultProvider);
  };

  const [defaultSaved, setDefaultSaved] = useState(false);
  const [hasStoredDefault, setHasStoredDefault] = useState(() => !!localStorage.getItem(STORAGE_KEY));

  const hasNonDefaultConfig = priority !== 0 || mode !== 'auto' || provider !== defaultProvider || model !== '' || effort !== '' || codexServiceTier !== 'default' || thinkingBudget !== '' || timeoutHours !== '' || systemPromptMode !== '';
