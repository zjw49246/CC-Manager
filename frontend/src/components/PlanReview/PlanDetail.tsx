  ...
  return <div className="flex h-full min-h-0 min-w-0 flex-col overflow-x-hidden">
    ...
    <div className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {plan.target_task_id != null && onNavigateTask && <button type="button" disabled={busy} onClick={() => { onNavigateTask(plan.target_task_id!); onClose?.(); }} className="rounded-lg border border-indigo-500/40 px-3 py-2 text-xs text-indigo-300 transition-colors hover:border-indigo-400/60 hover:bg-indigo-500/10 disabled:pointer-events-none disabled:opacity-40">Open related Task #{plan.target_task_id}</button>}
        ...
        {activeRun && <button type="button" disabled={busy} onClick={() => void mutate('Cancelling planning', () => api.cancelPlanRun(activeRun.id))} className="rounded-lg border border-red-500/40 px-3 py-2 text-xs text-red-300 transition-colors hover:border-red-400/60 hover:bg-red-500/10 disabled:pointer-events-none disabled:opacity-40">Cancel planning</button>}
